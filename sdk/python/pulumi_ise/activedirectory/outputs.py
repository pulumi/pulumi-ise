# coding=utf-8
# *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import copy
import warnings
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
from .. import _utilities

__all__ = [
    'AddGroupsGroup',
    'JoinDomainWithAllNodesAdditionalData',
    'JoinPointAttribute',
    'JoinPointGroup',
    'JoinPointRewriteRule',
    'GetGroupsByDomainGroupResult',
    'GetJoinPointAttributeResult',
    'GetJoinPointGroupResult',
    'GetJoinPointRewriteRuleResult',
]

@pulumi.output_type
class AddGroupsGroup(dict):
    def __init__(__self__, *,
                 name: str,
                 sid: str,
                 type: Optional[str] = None):
        """
        :param str name: Required for each group in the group list with no duplication between groups
        :param str sid: Required for each group in the group list with no duplication between groups
        """
        pulumi.set(__self__, "name", name)
        pulumi.set(__self__, "sid", sid)
        if type is not None:
            pulumi.set(__self__, "type", type)

    @property
    @pulumi.getter
    def name(self) -> str:
        """
        Required for each group in the group list with no duplication between groups
        """
        return pulumi.get(self, "name")

    @property
    @pulumi.getter
    def sid(self) -> str:
        """
        Required for each group in the group list with no duplication between groups
        """
        return pulumi.get(self, "sid")

    @property
    @pulumi.getter
    def type(self) -> Optional[str]:
        return pulumi.get(self, "type")


@pulumi.output_type
class JoinDomainWithAllNodesAdditionalData(dict):
    def __init__(__self__, *,
                 name: str,
                 value: str):
        """
        :param str name: Additional attribute name
        :param str value: Additional attribute value
        """
        pulumi.set(__self__, "name", name)
        pulumi.set(__self__, "value", value)

    @property
    @pulumi.getter
    def name(self) -> str:
        """
        Additional attribute name
        """
        return pulumi.get(self, "name")

    @property
    @pulumi.getter
    def value(self) -> str:
        """
        Additional attribute value
        """
        return pulumi.get(self, "value")


@pulumi.output_type
class JoinPointAttribute(dict):
    @staticmethod
    def __key_warning(key: str):
        suggest = None
        if key == "defaultValue":
            suggest = "default_value"
        elif key == "internalName":
            suggest = "internal_name"

        if suggest:
            pulumi.log.warn(f"Key '{key}' not found in JoinPointAttribute. Access the value via the '{suggest}' property getter instead.")

    def __getitem__(self, key: str) -> Any:
        JoinPointAttribute.__key_warning(key)
        return super().__getitem__(key)

    def get(self, key: str, default = None) -> Any:
        JoinPointAttribute.__key_warning(key)
        return super().get(key, default)

    def __init__(__self__, *,
                 default_value: str,
                 internal_name: str,
                 name: str,
                 type: str):
        """
        :param str default_value: Required for each attribute in the attribute list. Can contain an empty string.
        :param str internal_name: Required for each attribute in the attribute list
        :param str name: Required for each attribute in the attribute list with no duplication between attributes
        :param str type: Required for each group in the group list
                 - Choices: `STRING`, `IP`, `BOOLEAN`, `INT`, `OCTET_STRING`
        """
        pulumi.set(__self__, "default_value", default_value)
        pulumi.set(__self__, "internal_name", internal_name)
        pulumi.set(__self__, "name", name)
        pulumi.set(__self__, "type", type)

    @property
    @pulumi.getter(name="defaultValue")
    def default_value(self) -> str:
        """
        Required for each attribute in the attribute list. Can contain an empty string.
        """
        return pulumi.get(self, "default_value")

    @property
    @pulumi.getter(name="internalName")
    def internal_name(self) -> str:
        """
        Required for each attribute in the attribute list
        """
        return pulumi.get(self, "internal_name")

    @property
    @pulumi.getter
    def name(self) -> str:
        """
        Required for each attribute in the attribute list with no duplication between attributes
        """
        return pulumi.get(self, "name")

    @property
    @pulumi.getter
    def type(self) -> str:
        """
        Required for each group in the group list
          - Choices: `STRING`, `IP`, `BOOLEAN`, `INT`, `OCTET_STRING`
        """
        return pulumi.get(self, "type")


@pulumi.output_type
class JoinPointGroup(dict):
    def __init__(__self__, *,
                 name: str,
                 sid: str,
                 type: Optional[str] = None):
        """
        :param str name: Required for each group in the group list with no duplication between groups
        :param str sid: Required for each group in the group list with no duplication between groups
        """
        pulumi.set(__self__, "name", name)
        pulumi.set(__self__, "sid", sid)
        if type is not None:
            pulumi.set(__self__, "type", type)

    @property
    @pulumi.getter
    def name(self) -> str:
        """
        Required for each group in the group list with no duplication between groups
        """
        return pulumi.get(self, "name")

    @property
    @pulumi.getter
    def sid(self) -> str:
        """
        Required for each group in the group list with no duplication between groups
        """
        return pulumi.get(self, "sid")

    @property
    @pulumi.getter
    def type(self) -> Optional[str]:
        return pulumi.get(self, "type")


@pulumi.output_type
class JoinPointRewriteRule(dict):
    @staticmethod
    def __key_warning(key: str):
        suggest = None
        if key == "rewriteMatch":
            suggest = "rewrite_match"
        elif key == "rewriteResult":
            suggest = "rewrite_result"
        elif key == "rowId":
            suggest = "row_id"

        if suggest:
            pulumi.log.warn(f"Key '{key}' not found in JoinPointRewriteRule. Access the value via the '{suggest}' property getter instead.")

    def __getitem__(self, key: str) -> Any:
        JoinPointRewriteRule.__key_warning(key)
        return super().__getitem__(key)

    def get(self, key: str, default = None) -> Any:
        JoinPointRewriteRule.__key_warning(key)
        return super().get(key, default)

    def __init__(__self__, *,
                 rewrite_match: str,
                 rewrite_result: str,
                 row_id: str):
        """
        :param str rewrite_match: Required for each rule in the list with no duplication between rules
        :param str rewrite_result: Required for each rule in the list
        :param str row_id: Required for each rule in the list in serial order
        """
        pulumi.set(__self__, "rewrite_match", rewrite_match)
        pulumi.set(__self__, "rewrite_result", rewrite_result)
        pulumi.set(__self__, "row_id", row_id)

    @property
    @pulumi.getter(name="rewriteMatch")
    def rewrite_match(self) -> str:
        """
        Required for each rule in the list with no duplication between rules
        """
        return pulumi.get(self, "rewrite_match")

    @property
    @pulumi.getter(name="rewriteResult")
    def rewrite_result(self) -> str:
        """
        Required for each rule in the list
        """
        return pulumi.get(self, "rewrite_result")

    @property
    @pulumi.getter(name="rowId")
    def row_id(self) -> str:
        """
        Required for each rule in the list in serial order
        """
        return pulumi.get(self, "row_id")


@pulumi.output_type
class GetGroupsByDomainGroupResult(dict):
    def __init__(__self__, *,
                 name: str,
                 sid: str,
                 type: str):
        """
        :param str name: Group name
        :param str sid: Group SID
        :param str type: Group type
        """
        pulumi.set(__self__, "name", name)
        pulumi.set(__self__, "sid", sid)
        pulumi.set(__self__, "type", type)

    @property
    @pulumi.getter
    def name(self) -> str:
        """
        Group name
        """
        return pulumi.get(self, "name")

    @property
    @pulumi.getter
    def sid(self) -> str:
        """
        Group SID
        """
        return pulumi.get(self, "sid")

    @property
    @pulumi.getter
    def type(self) -> str:
        """
        Group type
        """
        return pulumi.get(self, "type")


@pulumi.output_type
class GetJoinPointAttributeResult(dict):
    def __init__(__self__, *,
                 default_value: str,
                 internal_name: str,
                 name: str,
                 type: str):
        """
        :param str default_value: Required for each attribute in the attribute list. Can contain an empty string.
        :param str internal_name: Required for each attribute in the attribute list
        :param str name: Required for each attribute in the attribute list with no duplication between attributes
        :param str type: Required for each group in the group list
        """
        pulumi.set(__self__, "default_value", default_value)
        pulumi.set(__self__, "internal_name", internal_name)
        pulumi.set(__self__, "name", name)
        pulumi.set(__self__, "type", type)

    @property
    @pulumi.getter(name="defaultValue")
    def default_value(self) -> str:
        """
        Required for each attribute in the attribute list. Can contain an empty string.
        """
        return pulumi.get(self, "default_value")

    @property
    @pulumi.getter(name="internalName")
    def internal_name(self) -> str:
        """
        Required for each attribute in the attribute list
        """
        return pulumi.get(self, "internal_name")

    @property
    @pulumi.getter
    def name(self) -> str:
        """
        Required for each attribute in the attribute list with no duplication between attributes
        """
        return pulumi.get(self, "name")

    @property
    @pulumi.getter
    def type(self) -> str:
        """
        Required for each group in the group list
        """
        return pulumi.get(self, "type")


@pulumi.output_type
class GetJoinPointGroupResult(dict):
    def __init__(__self__, *,
                 name: str,
                 sid: str,
                 type: str):
        """
        :param str name: Required for each group in the group list with no duplication between groups
        :param str sid: Required for each group in the group list with no duplication between groups
        """
        pulumi.set(__self__, "name", name)
        pulumi.set(__self__, "sid", sid)
        pulumi.set(__self__, "type", type)

    @property
    @pulumi.getter
    def name(self) -> str:
        """
        Required for each group in the group list with no duplication between groups
        """
        return pulumi.get(self, "name")

    @property
    @pulumi.getter
    def sid(self) -> str:
        """
        Required for each group in the group list with no duplication between groups
        """
        return pulumi.get(self, "sid")

    @property
    @pulumi.getter
    def type(self) -> str:
        return pulumi.get(self, "type")


@pulumi.output_type
class GetJoinPointRewriteRuleResult(dict):
    def __init__(__self__, *,
                 rewrite_match: str,
                 rewrite_result: str,
                 row_id: str):
        """
        :param str rewrite_match: Required for each rule in the list with no duplication between rules
        :param str rewrite_result: Required for each rule in the list
        :param str row_id: Required for each rule in the list in serial order
        """
        pulumi.set(__self__, "rewrite_match", rewrite_match)
        pulumi.set(__self__, "rewrite_result", rewrite_result)
        pulumi.set(__self__, "row_id", row_id)

    @property
    @pulumi.getter(name="rewriteMatch")
    def rewrite_match(self) -> str:
        """
        Required for each rule in the list with no duplication between rules
        """
        return pulumi.get(self, "rewrite_match")

    @property
    @pulumi.getter(name="rewriteResult")
    def rewrite_result(self) -> str:
        """
        Required for each rule in the list
        """
        return pulumi.get(self, "rewrite_result")

    @property
    @pulumi.getter(name="rowId")
    def row_id(self) -> str:
        """
        Required for each rule in the list in serial order
        """
        return pulumi.get(self, "row_id")


